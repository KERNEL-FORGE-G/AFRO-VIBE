  ...
  const handleCapture = async () => {
    if (captureMode === 'photo') {
      try {
        ...
        const mediaUri = path.startsWith('file://') ? path : `file://${path}`;
    ...
    try {
      ...
      setRecording(true);
      recorder.current = await videoOutput.createRecorder();
      await recorder.current.startRecording(
        (videoPath) => {
          console.log('Recording finished:', videoPath);
          setRecording(false);
          setLoading(false);
          const mediaUri = videoPath.startsWith('file://') ? videoPath : `file://${videoPath}`;
